...
window.injectExtraControls = function() {
    ...
    if (!document.getElementById('jb-volume-group')) {
        ...
        volWrapper.innerHTML = `
            ...
            <input type="range" class="jb-volume-slider" id="jb-volume-slider" min="0" max="100" value="100" title="Volumen">
        `;
        toolsRow.appendChild(volWrapper);
        
        // Add Listeners
        setTimeout(() => {
            document.getElementById('jb-volume-slider').oninput = (e) => window.setJukeboxVolume(e.target.value);
            document.getElementById('jb-mute-btn').onclick = window.toggleJukeboxMute;
        }, 100);
    }
...
window.toggleJukeboxMute = function() {
    ...
    window.updateMuteIcon();
    window.setJukeboxVolume(currentVolume); // Aplicar
};
